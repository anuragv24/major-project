...
const Sidebar = () => {
    ...
    return (
        <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white custom-scrollbar ${selectedUser ? "max-md:hidden" : ""}`}>
            <div className="pb-5">
                <div className="flex justify-between items-center">
                    <img src={assets.logo || null} alt="logo" className="max-w-[120px] md:max-w-40" />
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        <div className="relative py-2 group">
                            ...
                            <div className="absolute top-full right-0 z-20 w-32 p-3 rounded-md bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:block shadow-xl">
                                <p onClick={() => navigate('/profile')} className="cursor-pointer text-sm p-2 hover:bg-white/5 rounded">Edit Profile</p>
                                <hr className='my-1 border-t border-gray-500 '/>
                                <p onClick={() => logout()} className='cursor-pointer text-sm p-2 hover:bg-white/5 rounded text-red-400'>Logout</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div className="flex flex-col gap-3">
                {filteredUsers.map((user) => (
                    <div 
                        ...
                        key={user._id}
                        className={`relative flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-all ${selectedUser?._id === user._id ? "bg-[#282142]" : "hover:bg-[#282142]/40"} `} 
                    >
                        <div className="relative">
                            <img src={user?.profilePic || assets.avatar_icon} alt="UserPic" className='w-10 h-10 rounded-full object-cover border border-white/10' />
                        ...
                        <div className="flex flex-col flex-1 overflow-hidden">
                            ...
                            <span className={`text-[10px] ${onlineUser.includes(user._id) ? 'text-green-400' : 'text-neutral-400'}`}>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
                    <div className="bg-[#282142] w-full max-w-sm overflow-hidden rounded-2xl border border-gray-600 shadow-2xl flex flex-col max-h-[85vh]">
                        ...
                        <div className="p-4 overflow-y-auto custom-scrollbar">
                            ...
                            <div className="mb-6">
                                <p className="text-[10px] text-gray-400 uppercase font-bold tracking-widest mb-2 px-1">Selected</p>
                            ...
                            <div className="grid grid-cols-1 gap-1">
                                {SARVAM_LANGUAGES.map((lang) => (
                                    ...
                                    >
                                        {lang.name}
                                        {authUser?.preferredLanguage === lang.code && <span className="text-[10px] font-bold">CURRENT</span>}
                                    </button>
                                ))}
                            </div>
                        </div>
