...
export const AuthProvider = ({ children }) => {
  ...
  const updateLanguage = async (lang) => {
    try {
      const { data } = await axios.put("/api/auth/update-language", {
  ...
  // send OTP function to handle sending OTP for registration and password reset
  const sendOtp = async (email, type = "registration") => {
    try {
      const { data } = await axios.post("/api/auth/send-otp", { email, type });
      if (data.success) {
  ...
  // verify OTP function to handle OTP verification for registration and password reset
  ...
  // reset password function to handle password reset using OTP
